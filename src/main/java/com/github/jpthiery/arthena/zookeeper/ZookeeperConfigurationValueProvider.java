...
public class ZookeeperConfigurationValueProvider implements ConfigurationValueProvider {
  ...
  public ZookeeperConfigurationValueProvider(ZooKeeper zookeeper, JsonConverter jsonConverter) {
    requireNonNull(zookeeper, "zookeeper must be defined");
    this.zookeeperClient = new ZookeeperClient(zookeeper);
    this.jsonConverter = requireNonNullElseGet(jsonConverter, DefaultJsonConverter::new);
  }
  ...
  public <T> Optional<T> getValue(ConfigurationKey key, Environment environment, Class<T> tClass) {
    ...
    var configuration =
        jsonConverter.configurationFromJsonByte(
    ...
    var configurationEntry = jsonConverter.configurationEntryFromJsonByte(valueContent, tClass);
  ...
  private <T> void convertDataToConfigurationEntryAndNotify(
    ...
    if (previous != null && previous.length > 0) {
        previousEntry =
            jsonConverter.configurationEntryFromJsonByte(previous,tClass);
    }
    ConfigurationEntry<T> currentEntry = null;
    if (newData != null && newData.length > 0) {
      currentEntry =
              jsonConverter.configurationEntryFromJsonByte(newData,tClass);
    }
